...
main(int argc, char *argv[])
{
    ...
    static core_results results[MULTITHREAD];  /* STATIC: avoid stack overflow on bare-metal */
...
#error "Please define a way to initialize a memory block."
    ...
    if (results[0].iterations == 0)
    {
        ...
        if (divisor == 0) /* some machines cast float to int as 0 since this
                             conversion is not defined by ANSI, but we know
...
#else
        ee_printf(
            "NOTE: Short run (simulation). CM/MHz computed from raw ticks.\n");
#endif
    }
